...
echo "<table>";
echo "<tr><th>Name</th><th>Successful Prints</th><th>Invention Runs</th><th>Percent</th></tr>";

while($line = mysql_fetch_assoc($db->result))
{
    printf("<tr><td><em>%s</em></td><td>%s</td><td>%s</td><td>%.2s%%</td></tr>", $line["typeName"], $line["succ"], $line["cnt"], $line["succ"] / $line["cnt"] * 100);
}
